...
namespace HotelDBWebserviceDemo.Controllers
{
    public class RoomsController : ApiController
    {
        ...
        [Route("api/rooms/{HotelNo:int}/{RoomNo:int}/HotelRoom")]
        [HttpGet]
        [ResponseType(typeof(Room))]
        public async Task<IHttpActionResult> GetRoomByHotel(int RoomNo, int HotelNo)
        {
            Room room = db.Room.Where(x => x.Room_No.Equals(RoomNo) && x.Hotel_No.Equals(HotelNo)).FirstOrDefault();
            if (room == null)
            {
                return NotFound();
            }
